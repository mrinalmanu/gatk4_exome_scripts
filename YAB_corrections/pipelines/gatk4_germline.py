...
#########################################################################################################################
	...
	def run_in_docker(self, cmd, image, threads_needed, stdout=None, stderr=None):
		...
		while wait_go !='go':
			time.sleep(10)
			wait_go = lock_module.check_threads(batch_ID, container_name,"start",(-1)*threads_needed, max_nr_threads)	
		...
		while wait_go !='go':
			time.sleep(10)
			wait_go = lock_module.check_threads(batch_ID, container_name,"finish", threads_needed, max_nr_threads)	
	...
	def run_pipeline(self):
		...
		def markDuplicates(input_bam):
			...
			gatk_docker("gatk_mark_duplicates",parameters_dict,
						markDuplicates_log, self.ram,self.docker_images_dict["broadinstitute/gatk"])
		...
		def addReadGroups():
			...
			gatk_docker("gatk_add_read_groups", parameters_dict,
						ReadGroups_log, self.ram,self.docker_images_dict["broadinstitute/gatk"])
		...
		def buildRecalibrator():
			...
			gatk_docker("gatk_build_recalibrator", parameters_dict,
						BaseRecalibrator_log, self.ram,self.docker_images_dict["broadinstitute/gatk"])
		...
		def applyRecalibrator():
			...
			gatk_docker("gatk_apply_recalibrator", parameters_dict,
						ApplyBQSR_log, self.ram,self.docker_images_dict["broadinstitute/gatk"])
			...
			gatk_docker("gatk_haplotype_caller", parameters_dict,
						HaplotypeCaller_log, self.ram,self.docker_images_dict["broadinstitute/gatk"])
